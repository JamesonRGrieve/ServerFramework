...
def test_regenerate_uses_db_info_file_path(migration_manager, tmp_path):
    ...
    src_path = Path(__file__).resolve().parent.parent.parent
    decoy = src_path / "database" / "serverframework.database.db"
...
def test_create_cli_verb_dispatches_to_create_extension(capsys):
    ...
    from serverframework.database.migrations import Migration as mig_mod

    assert hasattr(mig_mod.MigrationManager, "create_extension")

    # Migration.py lives at <src>/serverframework/database/migrations/.
    # Climb four parents to reach <src>; pass that on PYTHONPATH so the
    # subprocess can resolve ``serverframework.*`` imports.
...
def test_item62_global_paths_override_picked_up(tmp_path):
    ...
    from serverframework.database.migrations.Migration import MigrationManager
    from serverframework.lib import Paths
