...
from serverframework.database.DatabaseManager import DatabaseManager, db_name_to_path
...
class TestCreatePermissionReference:
    ...
    def test_multiple_permission_references_without_create(self, mock_db):
        ...
        from serverframework.database.StaticPermissions import user_can_create_referenced_entity
        ...
        with patch("serverframework.database.StaticPermissions.is_root_id", return_value=True):
        ...
        with patch("serverframework.database.StaticPermissions.is_root_id", return_value=False):
            result, error = user_can_create_referenced_entity(
    ...
    def test_explicit_create_permission_reference(self, mock_db):
        ...
        with patch("serverframework.database.StaticPermissions.is_root_id", return_value=False), patch(
            "serverframework.database.StaticPermissions.find_create_permission_reference_chain",
            return_value=(ExplicitRefClass, None),
        ), patch("serverframework.database.StaticPermissions.user_can_edit", return_value=True):
    ...
    def test_find_create_permission_reference_chain(self, mock_db):
        ...
        from serverframework.database.StaticPermissions import find_create_permission_reference_chain
        ...
        with patch("serverframework.database.StaticPermissions.getattr") as mock_getattr:
...
class TestTeamDepthLimit:
    def test_team_depth_limit(self, mock_db, test_records):
        ...
        from serverframework.database import StaticPermissions
...
class TestRequiredReferences:
    def test_denies_missing_required_references(self, mock_db, test_records):
        ...
        from serverframework.database.StaticPermissions import check_access_to_all_referenced_entities
...
class TestRecursiveReferenceProtection:
    def test_circular_reference_detection_in_get_referenced_records(self):
        ...
        from serverframework.database.StaticPermissions import get_referenced_records
    ...
    def test_circular_reference_detection_in_generate_permission_filter(self, mock_db):
        ...
        from serverframework.database.StaticPermissions import generate_permission_filter
        ...
        with patch("serverframework.database.StaticPermissions.inspect", side_effect=mock_inspect):
...
class TestTeamHierarchyDepth:
    def test_max_team_hierarchy_depth(self, mock_db):
        ...
        from serverframework.database.StaticPermissions import _get_admin_accessible_team_ids_cte
...
class TestRoleHierarchyProtection:
    def test_role_hierarchy_map_has_limits(self, mock_db):
        ...
        from serverframework.database.StaticPermissions import _get_role_hierarchy_map
...
class TestNullChecks:
    def test_check_permission_handles_nulls(self, mock_db, test_records):
        ...
        from serverframework.database.StaticPermissions import PermissionResult
    ...
    def test_check_permission_handles_deleted_at_safely(self, mock_db, test_records):
        ...
        with patch("serverframework.database.StaticPermissions.is_root_id", return_value=False), patch(
            "serverframework.database.StaticPermissions.is_system_id", return_value=False
        ), patch(
            "serverframework.database.StaticPermissions.exists",
            return_value=mock_db.query.return_value.exists.return_value,
        ), patch(
            "serverframework.database.StaticPermissions.generate_permission_filter", return_value=True
        ), patch(
            f"{__name__}.check_permission_wrapper",
            return_value=(PermissionResult.GRANTED, None),
        ):
