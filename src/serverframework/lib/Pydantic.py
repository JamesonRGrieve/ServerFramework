...
from serverframework.lib.Pydantic2FastAPI import generate_routers_from_model_registry
...
from serverframework.lib.AbstractPydantic2 import (
    CacheManager,
    FieldProcessor,
    NameProcessor,
    ReferenceResolver,
...
from serverframework.lib.Environment import AbstractRegistry, env, inflection
...
class ModelRegistry(AbstractRegistry):
    ...
    def _auto_bind_models(self, extensions_str: str = ""):
        ...
        # IS ``<src>/serverframework`` after Item 60). The canonical
        # ``serverframework.`` prefix is added back at module-name
    ...
    def _process_extensions(self) -> None:
        ...
        from serverframework.lib.Logging import logger
        from serverframework.lib.Pydantic2SQLAlchemy import _apply_model_extension
    ...
    def _seed(self) -> None:
        ...
        """
        from serverframework.database.StaticSeeder import seed_model
    ...
    def _stamp_extension_table_ownership(self) -> None:
        ...
        def _ext_name(module_path: str) -> Optional[str]:
            ...
            # Canonical post-Item-60 form: ``serverframework.extensions.<name>...``
            if (
                len(parts) >= 3
                and parts[0] == "serverframework"
    ...
    def _scoped_import(
        ...
        for module_name in sys.modules:
            if (
                module_name.startswith("serverframework.database.DB_")
                or module_name.startswith("serverframework.extensions.")
        ...
        for file_path in ordered_files:
            ...
            # Module names are made canonical (rooted at ``serverframework.``)
            ...
            for scope, files in files_by_scope.items():
                if file_path in files:
                    ...
                    canonical = f"serverframework.{scope}.{stem}" if not scope.startswith("serverframework.") else f"{scope}.{stem}"
            ...
            try:
                ...
                if "serverframework.database.DB_" in module_name and module_name in sys.modules:
                    logger.debug(f"Reusing already imported core module: {module_name}")
                    module = sys.modules[module_name]
                elif (module_name.startswith("serverframework.extensions.") or module_name.startswith("extensions.")):
                    ...
                    # Canonical form is ``serverframework.extensions.<ext_name>.<file_stem>``;
                    ...
                    # ``from serverframework.logic.BLL_X import Y``),
    ...
    def _parse_imports_and_dependencies(self, file_path, scope="database"):
        ...
        Args:
            ...
            scope: The module scope (e.g., "database" or "serverframework.extensions.prompts")
    ...
    def _load_extension_ep_files(self, extension_names):
        ...
        for ext_name in extension_names:
            ...
            for file_path in ep_files:
                module_name = (
                    f"serverframework.extensions.{ext_name}.{os.path.basename(file_path)[:-3]}"
                )
    ...
    def from_scoped_import(
        ...
        Args:
            ...
            scopes: List of scopes to search (e.g., ["logic", "serverframework.extensions.payment"])
...
def obj_to_dict(obj, _visited=None):
    ...
    if isinstance(obj, datetime):
        ...
        try:
            from serverframework.lib.RequestContext import get_user_timezone
