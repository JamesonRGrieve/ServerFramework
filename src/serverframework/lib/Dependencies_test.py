...
class TestDependencies(unittest.TestCase):
    ...
    def test_sys_dependency_is_satisfied(self, mock_run):
        ...
        with patch("serverframework.lib.Dependencies.get_os_type", return_value=OSType.UBUNTU):
            with patch(
                "serverframework.lib.Dependencies.APTPackageManager.is_available", return_value=True
            ):
                with patch(
                    "serverframework.lib.Dependencies.APTPackageManager.check_package_installed",
                    return_value=True,
                ):
                    mappings = [
                        SystemPackageMapping(manager="apt", package_name="test-apt")
    ...
    @patch("serverframework.lib.Dependencies.HAS_DISTRO", False)
    ...
    @patch("serverframework.lib.Dependencies.execute_command")
    ...
    def test_install_system_dependencies(self):
        ...
        with patch(
            "serverframework.lib.Dependencies.check_system_dependencies", return_value=mock_check
        ):
            # Mock dependency resolution to return the same dependencies
            with patch(
                "serverframework.lib.Dependencies.resolve_dependencies",
                return_value={dep.name: dep for dep in deps},
            ):
                # Mock the _install_apt_dependencies function
                with patch(
                    "serverframework.lib.Dependencies._install_apt_dependencies",
                    return_value=expected_result,
                ) as mock_apt:
                    # Also mock get_os_type to ensure OS detection works
                    with patch(
                        "serverframework.lib.Dependencies.get_os_type", return_value=OSType.UBUNTU
                    ):
                        ...
                        with patch(
                            "serverframework.lib.Dependencies.get_available_package_managers",
    ...
    def test_install_pip_dependencies_with_uv(self, mock_which, mock_run):
        ...
        with patch(
            "serverframework.lib.Dependencies.check_pip_dependencies",
    ...
    def test_snap_package_manager(self, mock_execute):
        ...
        from serverframework.lib.Dependencies import SnapPackageManager
    ...
    def test_chocolatey_package_manager(self, mock_execute):
        ...
        from serverframework.lib.Dependencies import ChocolateyPackageManager
    ...
    def test_brew_package_manager(self, mock_execute):
        ...
        from serverframework.lib.Dependencies import BrewPackageManager
    ...
    def test_winget_package_manager(self, mock_execute):
        ...
        from serverframework.lib.Dependencies import WinGetPackageManager
    ...
    def test_version_compatibility_edge_cases(self):
        ...
        with patch("serverframework.lib.Dependencies.check_version_compatibility") as mock_check:
    ...
    def test_dependency_resolution(self):
        ...
        # and mock it instead
        with patch("serverframework.lib.Dependencies.Resolver") as mock_resolver_class:
            ...
            with patch("serverframework.lib.Dependencies.BaseReporter", MagicMock()):
        ...
        with patch(
            "serverframework.lib.Dependencies.Resolver", side_effect=Exception("Resolution error")
        ):
            with self.assertRaises(DependencyResolutionError):
                resolve_dependencies(deps_dict)

    @patch("platform.system")
    @patch("serverframework.lib.Dependencies.HAS_DISTRO", True)
    def test_os_detection_with_distro(self, mock_system):
        """Test OS detection using the distro package."""
        # Set up for Linux tests
        mock_system.return_value = "Linux"

        with patch("serverframework.lib.Dependencies.distro.id") as mock_id, patch(
            "serverframework.lib.Dependencies.distro.like"
        ) as mock_like:
    ...
    def test_install_with_dependency_resolution(self):
        ...
        with patch("serverframework.lib.Dependencies.resolve_dependencies") as mock_resolve:
    ...
    def test_complex_dependency_resolution(self):
        ...
        # Skip if resolvelib is not available
        if (
            not hasattr(sys.modules["serverframework.lib.Dependencies"], "HAS_RESOLVELIB")
            or not sys.modules["serverframework.lib.Dependencies"].HAS_RESOLVELIB
        ):
            self.skipTest("resolvelib not available")
        ...
        with patch("serverframework.lib.Dependencies.DependencyProvider", return_value=provider):
            with patch("serverframework.lib.Dependencies.Resolver") as mock_resolver_class:
    ...
    def test_resolve_dependencies_without_resolvelib(self):
        ...
        with patch("serverframework.lib.Dependencies.HAS_RESOLVELIB", False):
...
class TestExtDependencyOptionalMissing(unittest.TestCase):
    ...
    def test_default_callback_logs_warning_when_optional_missing(self):
        ...
        with patch("serverframework.lib.Dependencies.logger") as mock_logger:
...
class TestPrintOptionalSkipBanner(unittest.TestCase):
    ...
    def test_banner_lists_each_skipped_dep(self):
        from serverframework.lib.Dependencies import print_optional_skip_banner
