...
class AbstractDatabaseMemoryProvider(AbstractStaticProvider):
    ...
    @classmethod
    def build_streams_transport(cls, instance, consumer_group="zephyrex") -> Any: ...
```
...
`AbstractDatabaseMemoryProvider.build_streams_transport(instance, consumer_group="zephyrex")` returns a `BrokerTransport`-shaped object the framework's EventBus consumes. The transport conforms to the `zephyrex.logic.EventBus.BrokerTransport` ABC structurally:
...
```python
from zephyrex.extensions.database_memory.EXT_DatabaseMemory import EXT_DatabaseMemory
from zephyrex.logic.EventBus import RedisStreamsEventBus, set_event_bus
...
transport = provider.build_streams_transport(instance, consumer_group="zephyrex")
