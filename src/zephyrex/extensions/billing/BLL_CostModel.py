...
class TenantCostCap:
    ...
    def __post_init__(self) -> None:
        ...
        # Validate the window string at construction time. The same syntax
        # is shared with audit_retention but billing does not depend on it.
        _validate_window(self.window)


_WINDOW_SUFFIX_SECONDS = {"s": 1, "m": 60, "h": 3600, "d": 86400, "y": 31_536_000}
_WINDOW_RE_BILLING = __import__("re").compile(r"^(\d+)([smhdy])$")


def _validate_window(window: str) -> None:
    if window == "forever":
        return
    if not _WINDOW_RE_BILLING.match(window):
        raise ValueError(
            f"Invalid window {window!r}; expected '<number><suffix>' "
            f"with suffix in {sorted(_WINDOW_SUFFIX_SECONDS.keys())} or 'forever'"
        )
