...
class SessionManager(AbstractBLLManager, RouterMixin):
    ...
    def get_user_sessions(
        ...
        return self.Model.DB(self.model_registry.DB.manager.Base).list(  # type: ignore[no-any-return]
            requester_id=self.requester.id,
            model_registry=self.model_registry,
            return_type="dto",
    ...
    def revoke_sessions(self, user_id: str) -> int:
        return self.revoke_all_user_sessions(user_id).get("revoked_count", 0)  # type: ignore[no-any-return]
