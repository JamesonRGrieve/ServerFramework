...
def _hash_secret(raw: str, salt: str) -> str:
    import bcrypt
    from zephyrex.logic.BLL_Auth import _BCRYPT_ROUNDS

    return bcrypt.hashpw((salt + raw).encode("utf-8"), bcrypt.gensalt(rounds=_BCRYPT_ROUNDS)).decode("utf-8")
...
class OAuth2AuthCodeManager(AbstractBLLManager, RouterMixin):
    ...
    def issue_code(
        ...
        return raw_code  # type: ignore[no-any-return]
...
class OAuth2TokenManager(AbstractBLLManager, RouterMixin):
    ...
    def has_scope(self, raw_token: str, required_scope: str) -> bool:
        ...
        return has_permission(  # type: ignore[no-any-return]
            required_scope,
            role_grants=set(),
