...
from zephyrex.database.StaticSeeder import seed_model, _resolve_placeholder_fields
...
def _load_extensions(extensions_list: str = ""):
    ...
    for ext_name in extension_names:
        ...
        ext_modules, ext_errors = scoped_import(
            file_type="EXT", 
            scopes=[f"zephyrex.extensions.{ext_name}"]
        )
```
...
def seed_data(cls) -> List[Dict[str, Any]]:
    ...
    from zephyrex.lib.Import import scoped_import
...
```python
import logging
logging.getLogger("zephyrex.database.StaticSeeder").setLevel(logging.DEBUG)
```
