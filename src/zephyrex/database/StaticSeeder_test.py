...
from zephyrex.database import StaticSeeder as static_seeder
...
def _get_semantic_identifier(seed_item: dict, fallback_index: int = 0) -> str:
    ...
    if "email" in seed_item and seed_item["email"]:
        ...
        if "@" in email:
            return email.split("@")[0].replace(".", "_")  # type: ignore[no-any-return]
        return email.replace(".", "_")  # type: ignore[no-any-return]
...
def _find_core_models_with_seed_data():
    ...
    for module_name, module in sys.modules.items():
        if module and module_name.startswith("zephyrex.logic.BLL_"):
...
class TestSeedDataGeneration:
    ...
    def test_provider_extension_model_uses_placeholders(self):
        ...
        from zephyrex.logic.BLL_Providers import ProviderExtensionModel
    ...
    def test_placeholder_resolution_handles_legacy_format(self):
        ...
        from zephyrex.database.DatabaseManager import DatabaseManager
        from zephyrex.database.StaticSeeder import _resolve_placeholder_fields
