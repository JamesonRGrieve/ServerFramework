...
-------------------------
:class:`ErrorReporter` is an ABC. Core ships :class:`NoopErrorReporter`
(the default) here; the concrete :class:`SentryErrorReporter` /
...
def _redaction_patcher(record):
    ...
    try:
        from zephyrex.lib.RequestContext import get_correlation_id
...
class _ContextEnrichingReporter(ErrorReporter):
    ...
    def report(self, exception: BaseException, context: Mapping[str, Any]) -> None:
        ...
        try:
            from zephyrex.lib.RequestContext import get_correlation_id, get_request_user
            ...
            pass
        try:
            from zephyrex.lib.ProviderHTTPClient import get_traceparent
