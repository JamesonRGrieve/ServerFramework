...
- Mock business logic or integration tests. No mocking of BLL managers, endpoint handlers, or extension functionality - use real database instances and real server connections instead. Unit testing of pure utility functions (SQL filter generation, permission calculations) may use mocks for isolation.
...
2. **Business Logic Layer (BLL_*.py)**: Core business logic managers with CRUD operations; includes endpoint generation via RouterMixin
3. **Extensions (EXT_*.py)**: Plugin system for adding modular functionality
4. **Providers (PRV_*.py)**: Standardized interfaces to external services
...
- `BLL_*.py`: Business logic managers (with RouterMixin for API endpoint generation)
- `EXT_*.py`: Extension modules
- `PRV_*.py`: Provider interfaces
- `EP_*_test.py`: Endpoint test files
- `*_test.py`: Test files (use abstract test base classes)

### Model Architecture

Each entity follows a three-model pattern:
- **Entity Model**: Core attributes and validation (define this with `ReferenceID` for relationships)
- **Reference Model**: Auto-generated by `ModelMeta` metaclass from `ReferenceID` fields
- **Network Model**: Auto-generated for API schemas (POST, PUT, SEARCH, Response classes)

Developers define the main EntityModel; Reference and Network classes are automatically generated. Manual creation is only needed for customization.
